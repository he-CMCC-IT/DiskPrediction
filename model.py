...
file_path = files_path + 'ST4000DM000_sort_use_sample_time_series_kDifferent_alpha07.csv'
...
p = np.where(data['failure'] == 1)[0][-1]
data_bad = data.iloc[:p + 1, :]
data_good = data.iloc[p + 1:, :]
del data

serial_bad = list(set(data_bad['serial_number']))
serial_good = list(set(data_good['serial_number']))
serial_good = serial_good[:500]
